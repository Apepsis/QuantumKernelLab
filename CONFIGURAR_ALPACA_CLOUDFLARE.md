...
| `ALLOWED_SYMBOLS` | La lista de `data/tickers.json`; el Worker V8 ya incluye los 33 símbolos por defecto |
...
Para comprobar desde la misma procedencia que usa GitHub Pages, abre la consola
del navegador dentro de la aplicación y ejecuta:

```js
fetch("https://investment-market-api.TU-SUBDOMINIO.workers.dev/health").then(r => r.json()).then(console.log)
```

Abrir `/health` en una pestaña separada confirma los secretos, pero no confirma
CORS. La prueba anterior sí utiliza el origen `https://apepsis.github.io`.
...
El Worker V8 normaliza automáticamente una barra o ruta accidental, pero sigue
siendo recomendable guardar solamente el origen. Después de cambiar una
variable en Cloudflare debes volver a desplegar el Worker.

### `/health` funciona, pero la aplicación no conecta

1. Confirma que la URL guardada en la aplicación sea solo la base del Worker;
   no debe terminar en `/health` ni `/quotes`.
2. Confirma `ALLOWED_ORIGINS=https://apepsis.github.io`.
3. Vuelve a desplegar el Worker actualizado. La versión V8 acepta hasta 50
   símbolos y contiene los 33 activos del Research Lab.
