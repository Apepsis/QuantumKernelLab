...
export type QuantumKernelMetric = {
  brierScore: number;
  logLoss: number;
  rocAuc: number | null;
  accuracy: number;
...
export type QuantumKernelAggregate = QuantumKernelMetric & {
  horizonSessions: number;
  model: "logistic" | "rbfSvm" | "quantumZZ" | string;
  folds: number;
};

export type QuantumKernelDataset = {
  schemaVersion: number;
  generatedAt: string;
  mode: "protocol" | "live";
  status: "awaiting-manual-run" | "completed";
  experimentId: string;
  title: string;
  hypothesis: string;
  ...
  design: {
    ...
    qubits: number;
    featureMap: { name: string; repetitions: number; entanglement: string; shots: number | null };
    sampling: Record<string, number>;
    baselines: string[];
    quantumModels: string[];
  };
  features: string[];
  aggregateResults: QuantumKernelAggregate[];
  foldResults: Array<{
    horizonSessions: number;
    testYear: number;
    ...
    encoder: { components: number; explainedVarianceRatio: number[]; angleRange: number[] };
    quantumCircuit: { backend: string; qubits: number; circuitDepth: number; circuitSize: number; elapsedSeconds: number; software: string };
    models: Array<{ name: string; family: string; metrics: QuantumKernelMetric; runtimeSeconds: number }>;
  }>;
  bootstrap: Array<{
    horizonSessions: number;
    ...
    ciLow: number | null;
    ciHigh: number | null;
    probabilityQuantumBetter: number | null;
  }>;
  governance: {
    role: "shadow-challenger";
    promotionPolicy: Record<string, number | boolean>;
    eligibleForPromotion: boolean;
    automaticPromotion: false;
    decision: string;
    horizonsPassed?: number;
    checks?: Array<Record<string, string | number | boolean>>;
  };
  reproducibility: {
    seed: number;
    configHash: string;
